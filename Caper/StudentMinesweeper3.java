...
public class StudentMinesweeper3 extends JFrame { //extends JFrame обеспечивает возникновение окошка на экране. Extends наследования от класса JFrame.

    private JLabel label;
    Color mainColor = new Color(149, 13, 119, 255);
    final int FIELD_SIZE = 20; // Размер нашего поля т.е. 12 на 12. т.е. 12 на 12, квадратиков.
    ...
    final int NUMBER_OF_MINES = 50; // Кол-во мин, т.е. 50.
    ...
    public StudentMinesweeper3() {  // Основная часть Сапёра.
       ...
       // setDefaultCloseOperation(EXIT_ON_CLOSE);  // Закрытие программы.
        ...
        canvas.addMouseListener(new MouseAdapter() {   // Прослушивание нажатие на мышь. MouseAdapter Ниже представлен код для него.
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                //Отрабатывания кода при нажатие ЛКМ(Левой Кнопки Мыши).
                    ...
                    if (e.getButton() == MOUSE_BUTTON_LEFT) // Левая кнопка мыши отжата, и мина не взорволась и не было победы, то...
                        if (field[y][x].isNotOpen()) {  // Если это ячейка не открыта, то...
                            ...
                            if (youWon) {
                                ...
                                frame1.setBounds(670, 270, 500, 415);
                                frame1.setVisible(true);
                                frame1.setContentPane(new Win3Panel());
                            }
                            ...
                            if (bangMine) {  // Вторая проверка. Если взорвалась мина, то...
                                ...
                                frame.setBounds(380, 230, 700, 380);
                                frame.setVisible(true);
                                frame.setContentPane(new Troll3Panel());
                            }
                        }



                    //Отрабатывания кода при нажатие ПКМ(Правой Кнопки Мыши).
    ...
    class Cell { // Клетка игрового поля.
        ...
        void Paint(Graphics g, int x, int y) { //
            g.setColor(Color.gray); // цвет разреза.
            g.drawRect(x*BLOCK_SIZE, y*BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE); // Рисует прямоугольничек, серый.
    ...
    class Troll3Panel extends JPanel{
        public void paintComponent(Graphics g){
            Image im = null;
            try {
                im = ImageIO.read(new File("C:\\Troll3.jpg"));
            } catch (IOException e) {}
    ...
    class Win3Panel extends JPanel{
        public void paintComponent(Graphics g){
            Image im = null;
            try {
                im = ImageIO.read(new File("C:\\Win3.jpg"));
            } catch (IOException e) {}
            g.drawImage(im, 0, 0, null);
